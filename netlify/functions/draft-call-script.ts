...
type CallScriptPayload = Record<ScriptKey, string>

type DraftCallScriptRequestBody = {
  lead_id?: unknown
}
...
function parseRequestBody(raw: string | null): DraftCallScriptRequestBody | null {
  if (!raw?.trim()) return null
  try {
    return JSON.parse(raw) as DraftCallScriptRequestBody
  } catch {
    return null
  }
}
...
function temperatureGuidance(temp: 'hot' | 'warm' | 'cold'): string {
  if (temp === 'hot') {
    return 'Hot lead: move toward scheduling a showing or next concrete step.'
  }
  if (temp === 'warm') {
    return 'Warm lead: understand timeline and keep the conversation moving.'
  }
  return 'Cold lead: low pressure, add value, invite a future conversation.'
}
...
function buildCallScriptPrompt(voiceProfile: string, lead: LeadRow): string {
  ...
  return [
    ...
    '2. reference: the specific detail from their inquiry or notes that makes this personal',
    '3. question_1: first question to understand their situation better',
    '4. question_2: follow-up question to qualify timeline',
    '5. close: how to end the call with a clear next step',
    '',
    'Rules:',
    '- Each bullet is one to two sentences.',
    '- Sounds like Jason, not a stiff corporate script.',
...
function parseCallScriptPayload(raw: unknown): CallScriptPayload | null {
  if (!raw || typeof raw !== 'object') return null
  const record = raw as Record<string, unknown>
  const result = {} as CallScriptPayload
...
async function callAnthropicForCallScript(prompt: string): Promise<CallScriptPayload> {
  ...
  const script = parseCallScriptPayload(parsed)
  if (!script) {
    throw new Error('Anthropic returned invalid call script JSON')
  }
  return script
}
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    try {
      ...
      return json(500, {
        code: 'internal_error',
        message: 'Failed to generate call script',
      })
    }
  } catch (err) {
